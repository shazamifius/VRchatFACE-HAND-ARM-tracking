...
impl CameraManager {
    ...
    pub fn start(&mut self, config: CameraConfig) -> Result<()> {
        ...
        // ------------------------------------------------------------------
        // Attempt 0 (PROVEN FAST PATH): let the driver pick its NATIVE default.
        //
        // On many MediaFoundation webcams, requesting a specific low-res NV12 format
        // negotiates a broken stream AND reports a bogus `frame_rate()` of 1, while
        // the driver's own default streams fine at ~30fps. The benchmark uses exactly
        // this request (`None`) and measured 27 FPS, so try it FIRST — and MEASURE the
        // real throughput instead of trusting the (often wrong) reported frame_rate.
        // ------------------------------------------------------------------
        {
            #[cfg(target_os = "windows")]
            ...
            match Camera::with_backend(index.clone(), req, backend) {
                Ok(mut cam) => {
                    if cam.open_stream().is_ok() {
                        // Measure ACTUAL delivered frames over ~700ms.
                        let mut got = 0u32;
                        let t0 = std::time::Instant::now();
                        while t0.elapsed().as_millis() < 700 {
                            if cam.frame().is_ok() { got += 1; }
                            if got >= 8 { break; }
                        }
                        let measured_fps = got as f32 / t0.elapsed().as_secs_f32().max(0.001);
                        let f = cam.camera_format();
                        if got >= 5 {
                            println!("[Rust] Attempt 0 (native default) OK: {}x{} {:?} — measured ~{:.0} fps (reported {})",
                                f.resolution().width(), f.resolution().height(), f.format(),
                                measured_fps, f.frame_rate());
                            self.camera = Some(cam);
                            self.current_config = Some(config);
                            return Ok(());
                        } else {
                            println!("[Rust] Attempt 0 native default too slow ({} frames in 700ms). Falling back to heuristics...", got);
                            let _ = cam.stop_stream();
                        }
                    }
                }
                Err(e) => {
                    println!("[Rust] Attempt 0 (native default) could not open: {}. Falling back to heuristics...", e);
                }
            }
        }
    ...
    pub fn test_all_cameras(&self) -> Vec<crate::tracking::types::CameraBenchmarkResult> {
        ...
        for cam_info in cameras {
            ...
            // Attempt 1: Open with high compatibility settings (Auto)
            // We want to see if we can get ANY stream.
            let req = RequestedFormat::new::<RgbFormat>(RequestedFormatType::None);
            
            // Function to perform the actual 3-second test on a given stream format
            let run_test = |req: RequestedFormat| -> Result<(String, u32, u32, f32, bool), String> {
                match Camera::new(index.clone(), req) {
                    Ok(mut cam) => {
                        match cam.open_stream() {
                            Ok(_) => {
                                let fmt = cam.camera_format();
                                let format_name = format!("{:?}", fmt.format());
                                ...
                                loop {
                                    ...
                                    match cam.frame() {
                                        ...
                                        Err(_) => {
                                            // Sleep slightly to prevent CPU spin if format is bad
                                ...
                                let success = fps > 1.0 && success_read;
                                
                                let _ = cam.stop_stream();
                                Ok((format_name, width, height, fps, success))
                            },
                            Err(e) => Err(format!("Stream Open Failed: {}", e))
                        }
                    },
                    Err(e) => Err(format!("Camera Access Failed: {}", e))
                }
            };

            match run_test(req) {
                Ok((fmt, w, h, fps, success)) => {
                    result.format = fmt.clone();
                    result.width = w;
                    result.height = h;
                    result.fps_actual = fps;
                    result.success = success;
                    println!("[Rust]   Result: {:.1} FPS (Format: {} {}x{})", fps, fmt, w, h);
                },
                Err(e) => {
                    println!("[Rust]   Main format test failed: {}. Attempting literal MJPEG fallback...", e);
                    
                    // Attempt 2: Explicit MJPEG request fallback just in case Auto fails entirely on a valid cam
                    let attempt_fmt = CameraFormat::new(Resolution::new(640, 480), FrameFormat::MJPEG, 30);
                    let mjpeg_req = RequestedFormat::new::<RgbFormat>(RequestedFormatType::Closest(attempt_fmt));
                    
                    match run_test(mjpeg_req) {
                        Ok((fmt, w, h, fps, success)) => {
                            result.format = fmt.clone();
                            result.width = w;
                            result.height = h;
                            result.fps_actual = fps;
                            result.success = success;
                            println!("[Rust]   Result (Fallback MJPEG): {:.1} FPS (Format: {} {}x{})", fps, fmt, w, h);
                        },
                        Err(e_mjpeg) => {
                            println!("[Rust]   Fallback MJPEG also failed: {}", e_mjpeg);
                            result.error = Some(e); // Return the original error
                        }
                    }
                }
            }
