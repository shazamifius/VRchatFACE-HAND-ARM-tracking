...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  setupEventListeners();
  await refreshCameras(); // auto-selects the best LOCAL camera
  // Start on the auto-selected local webcam (no forced phone mode).
  // To use the phone instead, pick "Phone Camera (Remote)" in the dropdown.
  startTracking();
});
...
async function refreshCameras() {
  elements.cameraSelect.innerHTML = '<option value="-1">Running 3s benchmark on all cameras...</option>';
  elements.cameraInfo.textContent = 'Running benchmark on cameras... This may take a while.';

  // Increase timeout substantially to allow 3 seconds per camera. Keep it safe at 60s.
  const timeout = new Promise((_, reject) => {
    setTimeout(() => reject(new Error("Camera benchmark timeout")), 60000);
  });

  try {
    const results = await Promise.race([
      invoke('run_camera_benchmark'),
      timeout
    ]);

    // Sort results by score (success first, then highest FPS, then resolution)
    results.sort((a, b) => {
      if (a.success !== b.success) return b.success ? 1 : -1;
      if (Math.abs(b.fps_actual - a.fps_actual) > 5) return b.fps_actual - a.fps_actual;
      const resA = a.width * a.height;
      const resB = b.width * b.height;
      return resB - resA;
    });

    cameras = results; // Save the benchmark results to the global cameras array
    updateCameraList(cameras);

    const passed = cameras.filter(c => c.success).length;
    elements.cameraInfo.textContent = `Found ${cameras.length} devices (${passed} passed test)`;

    // Auto-select the best camera if any passed
    ...
    console.error('Failed to run camera benchmark:', error);
    elements.cameraInfo.textContent = 'Benchmark failed. Showing manual options.';
    cameras = [];
    updateCameraList([]);
  }
}
...
function updateCameraList(cams) {
  ...
  if (hasDetected) {
    ...
    cams.forEach(cam => {
      ...
      const statusIcon = cam.success ? "🟢" : "🟠";
      const fpsText = cam.fps_actual > 0 ? `${cam.fps_actual.toFixed(1)}fps` : 'No Stream';
...
async function onCameraChange() {
  ...
  // If tracking is already running, restart it on the newly selected camera so
  // the switch takes effect immediately (the backend reads the camera at start).
  if (isTracking) {
    await stopTracking();
    await new Promise(r => setTimeout(r, 350)); // let threads + ports release
    await startTracking();
  }
}
